...
const two = [1, 3];
const doubled = two.map(function (item) {
  return item * 2;
});
...
const movieLengthsList = movies.map((item) => item.title.length);

const minLength = Math.min(...movieLengthsList);
console.log(`min title length = ${minLength} letters`);
const maxLength = Math.max(...movieLengthsList);
console.log(`max title length = ${maxLength} letters`);

const avgLengthThreshold = (maxLength - minLength) / 2 || 0;
console.log(`average title length = ${avgLengthThreshold}`);

const shortTitleThreshold =
  avgLengthThreshold - (avgLengthThreshold - minLength) / 2;
console.log(`short title threshold = ${shortTitleThreshold}`);

const longTitleThreshold =
  avgLengthThreshold + (maxLength - avgLengthThreshold) / 2;
console.log(`long title threshold = ${longTitleThreshold}`);

let shortMovies = [];
let longMovies = [];

for (let movie of movies) {
  if (movie.title.length > longTitleThreshold)
    longMovies.push(`"${movie.title}"`);
  else if (movie.title.length < shortTitleThreshold)
    shortMovies.push(`"${movie.title}"`);
}

console.log(`Movies with long title: ${longMovies.join("; ")}`);
console.log(`Movies with short title: ${shortMovies.join("; ")}`);
